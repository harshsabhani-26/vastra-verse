...
import { getShippingZone, updateShippingZone, type ShippingZoneData, type ShippingZoneType } from '../actions'
...
export default function EditZonePage({ params }: { params: { id: string } }) {
    ...
    const loadZone = async () => {
        setLoading(true)
        const result = await getShippingZone(params.id)
        if (result.success && result.zone) {
            ...
            setPincodesText((zone.pincodes as string[]).join(', '))
        }
        setLoading(false)
    }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const result = await updateShippingZone(params.id, {
                ...formData,
                pincodes
            })
    ...
    if (loading) {
        return <div className="text-center py-12">Loading zone...</div>
    }
